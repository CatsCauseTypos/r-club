...
ELF$flat<-(as.factor(ELF$flat)) #good
ELF$row<-(as.factor(ELF$row)) #good
ELF$diameter<-(as.numeric(ELF$diameter)) #that did bad things

ELF$diameter<-(as.numeric(as.character(ELF$diameter))) #NA introduced by coersion error = there was something scary R couldn't understand and so it turned that into an NA value. 

#so how do we know what it replaced? a) we could create code that says, "I expect __.__ in my diameter - tell me what doesn't look that way" or we could let the NA values replace it and backwards extrapilate the rows which are troublesome for evaluation from the original data set... see here:
ELF<-(ELF3s_bolting_Rclub)
ELF$diameter<-(as.numeric(as.character(ELF$diameter))) #NA introduced by coersion error = there was something scary R couldn't understand and so it turned that into an NA value. 
#look - here's the original line
ELF3s_bolting_Rclub[is.na(ELF$diameter),]

ELF$diameter[is.na(ELF$diameter)] <-43.68
#put 43.68 in elf$diameter cell IF the cell is NA

str(ELF) #to check if it's okay
...
#there are two Sh2.H4 guys - are there two plants in that one pot?
ELF3s_bolting_Rclub[ELF3s_bolting_Rclub$Plant=="Sh2.H4.1h",]
#it's the same plant.
#how do you kill just the one?
E<-unique(ELF3s_bolting_Rclub)

summary(E)
#no more duplicate plant
